...
ORIENTDB_DIR="/opt/cs/cs_IWP/orient/orientdb-community-2.2.22/"
...
start() {
	...
	cd "$ORIENTDB_DIR/bin"
	sudo -u $ORIENTDB_USER sh -c "cd \"$ORIENTDB_DIR/bin\"; /usr/bin/nohup ./server.sh 1>$LOG_DIR/orientdb.log 2>$LOG_DIR/orientdb.err &"
...
stop() {
	...
	cd "$ORIENTDB_DIR/bin"
	sudo -u $ORIENTDB_USER sh -c "cd \"$ORIENTDB_DIR/bin\"; /usr/bin/nohup ./shutdown.sh 1>>$LOG_DIR/orientdb.log 2>>$LOG_DIR/orientdb.err &"	
